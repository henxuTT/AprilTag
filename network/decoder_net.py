...
class KeypointRegressor(nn.Module):
    def __init__(self, num_channels=32, num_classes=3, num_keypoints=33, input_size=2048):
        super().__init__()
        self.extras = nn.Sequential(
            conv(num_channels, num_channels),
            conv(num_channels, num_channels),
        ...
        self.fc = nn.Linear(input_size, num_keypoints * 2)
    ...
    def forward(self, x):
        ...
        location = self.fc(location)
        location = location.view(location.size(0), self.num_keypoints, 2)
